...
* **Android (fix duplicate alert sound on unlock):** the looping FSI sound could keep playing in the **cached** lock-screen engine after the user unlocked, overlapping the host app's over-apps overlay sound (the "double sound"). Root cause: `onReset` (which stops the Dart sound) was only invoked by `finishAndCleanup()` — reached via dismiss/accept/`finishLiveInstanceFromHost`. But the **USER_PRESENT** receiver only launched the host app; the Activity was then destroyed by the OS, and since the pre-warmed engine is cached (`shouldDestroyEngineWithHost()==false`) its isolate — and its sound — survived `onDestroy()`, dying only at the 30s Dart expiry. Now: (1) `onDestroy()` is a **failsafe** that invokes `onReset` on the cached engine on **every** destruction path (USER_PRESENT, OS-reclaim, swipe-away) unless `finishAndCleanup()` already ran; and (2) the USER_PRESENT receiver calls `finishLiveInstanceFromHost()` after launching the host app, so the live Activity resets cleanly while its channel is still valid.
