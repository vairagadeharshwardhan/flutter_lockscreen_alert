...
* **Android (fix: on OEMs that block draw-over-keyguard the user got NOTHING — no card AND no tile):** on ROMs where "Show on lock screen" / background pop-up ships OFF with no API to grant (Xiaomi/MIUI, Oppo, Vivo, Realme, Transsion), `LockscreenAlertActivity` is occluded behind the secure keyguard and never becomes visible. It was cancelling the notification in `onStart()` (assuming the card was now on screen), leaving the user with nothing. Now the notification is KEPT as the guaranteed, tappable lock-screen alert on every device — still cancelled on the real teardown paths (accept / dismiss / host-finish / expiry) via `finishAndCleanup()`, so it never lingers. The full-screen card is now a best-effort ENHANCEMENT; the notification is the reliable floor.
* **Android:** the alert channel now sets an explicit insistent vibration pattern + lights (on top of `IMPORTANCE_MAX`'s default sound), so the notification alerts with haptics even when the full-screen card can't draw. (Channel importance/sound/vibration are locked after first creation, so this shapes fresh installs.)

## 1.0.19

* **Android (fix: lock-screen card never appears until the phone is unlocked):** `LockscreenAlertActivity` had **no `android:configChanges`** declared, so any configuration change destroyed and recreated it. When a booking arrives on a locked device, `setTurnScreenOn(true)` wakes the screen — and turning the screen on in dark mode applies a **`uiMode` → night** change, which tore the just-launched Activity down (pause → stop → destroy) within ~40 ms, before the user could see it; the recreated instance was lost behind the keyguard, so the full-screen card only surfaced after the user unlocked. The Activity now declares the standard Flutter set (`orientation|keyboardHidden|keyboard|screenSize|smallestScreenSize|locale|layoutDirection|fontScale|screenLayout|density|uiMode`), so it **handles config changes in-place instead of being recreated** and stays visible over the keyguard. (This also makes the 1.0.18 `isChangingConfigurations()` sound-stutter guard a no-op for these paths, since they no longer recreate the Activity at all.)
